...
def get_user_personal_scores(user_id: int, top_k: int = 50) -> Dict[int, float]:
    ...
    try:
        ...
        # Query explicit fields from feedback table (bookmark is a column in feedback, not a separate table)
        cur.execute("""
            SELECT ai_id,
                   feedback,
                   rating,
        ...
        logger.error("Error fetching personal scores: %s", e)
        return {}
    finally:
        if conn:
            conn.close()
...
def get_collaborative_scores(user_id: int, ai_data: List[Dict[str, Any]]) -> Dict[int, float]:
    ...
    try:
        ...
        logger.error("Error fetching collaborative scores: %s", e)
        return {}
    finally:
        if conn:
            conn.close()
...
def compute_engagement_scores(user_id: int, top_k: int = 50) -> Dict[int, float]:
    ...
    try:
        ...
        logger.error("Error fetching engagement scores: %s", e)
        return {}
    finally:
        if conn:
            conn.close()
