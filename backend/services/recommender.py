...
def deep_search_ai(input_text: str, top_k: int = 10) -> List[Dict[str, Any]]:
    ...
    logger.info("\n================= DEEP SEARCH START =================")
    logger.info("QUERY: %s", input_text)
    logger.info("TOP_K: %s", top_k)
    logger.info("=============================")

    input_text = (input_text or "").strip()
    if not input_text:
        logger.debug("EMPTY QUERY → RETURN []")
        return []
    ...
    try:
        ...
        # -------------------------------------------------------------
        logger.debug(">>> STEP 1: Semantic retrieval")
        try:
        ...
        logger.debug("Semantic candidates: %s", len(candidates))
        ...
        # -------------------------------------------------------------
        if not candidates:
            logger.debug(">>> STEP 2: Loading CSV fallback…")
            import pandas as pd
            ...
            logger.debug("Reading CSV: %s", DATA_PATH)
            df = pd.read_csv(DATA_PATH).fillna("")
            texts = df["Task Description"].astype(str).tolist()

            # ---- embeddings available?
            if Path(EMBEDDINGS_PATH).exists():
                logger.debug("Found embeddings: %s", EMBEDDINGS_PATH)
                try:
                    emb_matrix = np.load(EMBEDDINGS_PATH)
                    logger.debug("Loaded embedding matrix: %s", emb_matrix.shape)

                    # get embedding model
                    try:
                        model = globals().get("get_embedding_model", None)
                        emb_model = model() if callable(model) else SentenceTransformer("all-MiniLM-L6-v2")
                        logger.debug("Using embedding model: %s", emb_model)
                    except Exception:
                        emb_model = SentenceTransformer("all-MiniLM-L6-v2")
                        logger.debug("Fallback embedding model: MiniLM")

                    q_emb = emb_model.encode(input_text, convert_to_numpy=True)
                    logger.debug("Query embedding shape: %s", q_emb.shape)
                    ...
                    logger.debug("Similarity scores computed.")
                    top_idx = np.argsort(-sims)[:top_k]
                    logger.debug("Top indices: %s", top_idx)
                    ...
                    logger.warning("Embedding pipeline failed: %s", e)
                    candidates = []

            # ---- TF-IDF fallback ----
            if not candidates:
                logger.debug("Using TF-IDF fallback")
                try:
                    ...
                    logger.debug("TF-IDF top indices: %s", idxs)
            ...
            if not candidates:
                logger.warning("No candidates found → using fallback LLM")
                return normalize_records(llm_fallback(input_text))

        logger.debug(">>> FIRST-STAGE CANDIDATES:")
        for c in candidates:
            logger.debug("%s", c)
        ...
        # -------------------------------------------------------------
        logger.debug(">>> STEP 3: Creating LLM prompt")
...
"""
        ...
        # 4) CALL OLLAMA
        # -------------------------------------------------------------
        logger.debug(">>> STEP 4: Calling Ollama model: %s", OLLAMA_MODEL)
        try:
            result = subprocess.run(
            ...
            logger.info("Trying QUICK_MODEL: %s", QUICK_MODEL)
            try:
                result = subprocess.run(
                    ["ollama", "run", QUICK_MODEL, prompt],
                    capture_output=True, text=True, timeout=TIMEOUT
                ...
                logger.error("Both LLM calls failed: %s", e2)
                return normalize_records(llm_fallback(input_text))
        ...
        # 5) PARSE JSON
        # -------------------------------------------------------------
        logger.debug(">>> STEP 5: Parsing JSON output…")
        ...
        if not parsed:
            logger.warning("JSON parsing failed → using fallback ordering")
        ...
        # -------------------------------------------------------------
        logger.debug(">>> STEP 6: Mapping LLM IDs back to candidate objects")
        ...
        logger.debug("ID map keys: %s", list(by_id.keys()))
        logger.debug("Name map keys: %s", list(by_name.keys()))

        results = []

        for item in parsed:
            logger.debug("Processing LLM item: %s", item)

            # FIX: LLM returned bare number
            if isinstance(item, (int, float, str)):
                logger.debug("LLM item is bare number/string → converting")
                item = {
                    "ai_id": str(item),
                    "Name": "",
                    "Score": 1.0,
            ...
            try:
                ...
                logger.debug("Mapped fields: %s %s %s %s", aid, name, score, reason)

                match = None
                if aid in by_id:
                    logger.debug("Matched by ai_id")
                    match = dict(by_id[aid])
                elif name.lower() in by_name:
                    logger.debug("Matched by Name")
                    match = dict(by_name[name.lower()])
                else:
                    logger.debug("No match found → keeping LLM item")
        ...
        logger.debug("Mapped results so far:")
        for r in results:
            logger.debug("%s", r)
        ...
        logger.debug(">>> STEP 7: Padding missing items if needed")
        for c in csv_candidates:
            ...
            logger.debug("Adding padding item: %s", entry)
            results.append(entry)

        # -------------------------------------------------------------
        # 8) FINAL SORT
        # -------------------------------------------------------------
        logger.debug(">>> STEP 8: FINAL SORT BY SCORE")
