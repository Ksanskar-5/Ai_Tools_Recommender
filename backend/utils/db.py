...
Uses connection pooling for production reliability.
"""
    ...
    if _pool is None:
        try:
            _pool = pooling.MySQLConnectionPool(
                pool_name="ai_recommender_pool",
                pool_size=int(os.environ.get("DB_POOL_SIZE", "5")),
                pool_reset_session=True,
                host=os.environ.get("DB_HOST", "localhost"),
                user=os.environ.get("DB_USER", "root"),
                password=os.environ.get("DB_PASSWORD", ""),
                database=os.environ.get("DB_NAME", "ai_recommender"),
            )
...
def _ensure_tables():
    """Create required tables if they don't exist (runs once)."""
...
def get_all_ai() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"Error fetching AI data: {e}")
        return []
    finally:
        if conn:
            conn.close()
