...
- POST /api/feedback      → store like/dislike feedback (JWT required)
"""
...
def build_ai_map(ai_data):
    ...
    """
    return {row["Name"].strip().lower(): row["ai_id"] for row in ai_data if row.get("Name")}
...
def fix_ai_id(item, name_to_id):
    ...
    # Keep the item even without DB match — use existing ai_id or 0
    if not item.get("ai_id"):
        item["ai_id"] = 0
    return item
...
def recommend_route(req: RecommendRequest, request: Request):
    try:
        user = get_optional_user(request)
        user_id = user["user_id"] if user else (req.user_id or 0)
        ...
        # Step 4b: Engagement scoring
        engagement_scores = compute_engagement_scores(user_id) if user_id else {}
        ...
        for idx, item in enumerate(base_results):
            ...
            e_score = engagement_scores.get(ai_id, 0.0)

            if user_id:
                final_score = (0.6 * p_score) + (0.25 * c_score) + (0.15 * e_score)
            else:
                final_score = c_score
        ...
        # Step 7: Log implicit event (only for logged-in users)
        if user_id:
            for item in base_results:
                log_user_action(
                    user_id=user_id,
                    ai_id=item.get("ai_id", 0),
                    query=req.query,
                    action="search",
                    feedback=None
                )
...
def deep_search_route(req: RecommendRequest, request: Request):
    try:
        user = get_optional_user(request)
        user_id = user["user_id"] if user else (req.user_id or 0)
        ...
        # Step 4b: Engagement scores
        engagement_scores = compute_engagement_scores(user_id) if user_id else {}

        # Filter None entries before scoring
        deep_results = [i for i in deep_results if i]
        ...
        # Step 6: Log implicit deep search action (only for logged-in users)
        if user_id:
...
def feedback_endpoint(req: FeedbackRequest, user: dict = Depends(get_current_user)):
    try:
        if not req.ai_id:
...
def log_route(req: LogEventRequest, user: dict = Depends(get_current_user)):
    try:
        log_user_action(
            user_id=user["user_id"],
            ai_id=req.ai_id,
...
def bookmarks_route(user: dict = Depends(get_current_user)):
    user_id = user["user_id"]
    if not user_id:
        return {"bookmarks": []}
